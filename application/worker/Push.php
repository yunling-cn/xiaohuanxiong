    ...
    public function onConnect($connection)
    {
         try {
             ...
             if ($serverVersion > $localVersion) {
                 ...
                 $connection->send('<p style="padding-left:15px;font-weight: 400;color:#999;">覆盖版本号</p>');
                 for ($i = $localVersion + 1; $i <= $serverVersion; $i++) {
                     ...
                     if ((int)($res->getStatusCode()) == 200) {
                         ...
                         foreach ($json['update'] as $value) {
                             ...
                             $connection->send('<p style="padding-left:15px;font-weight: 400;color:#999;">升级文件' . $value . '</p>');
                         }
                         foreach ($json['delete'] as $value) {
                             $flag = unlink(Env::get('root_path') . '/' . $value);
                             if ($flag) {
                                 $connection->send('<p style="padding-left:15px;font-weight: 400;color:#999;">删除文件' . $value . '</p>');
                             } else {
                                 $connection->send('<p style="padding-left:15px;font-weight: 400;color:#999;">删除文件失败</p>');
                             }
                         }
                     }
                 }
